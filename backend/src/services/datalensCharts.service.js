...
function parseTimeSeriesChart(chartResponse, valueMode = 'amount') {
  const d = chartResponse?.data;
  if (!d) return [];
  ...
  let netPoints = [];
  if (valueMode === 'streams') {
    for (const group of seriesArr) {
      const groupArr = Array.isArray(group) ? group : [group];
      const streamsSeries = groupArr.find((s) => {
        const name = String(s?.name || '').toLowerCase();
        return name.includes('stream') || name.includes('стрим') || name.includes('прослуш');
      });
      if (streamsSeries) { netPoints = streamsSeries.data || []; break; }
    }
  }

  for (const group of seriesArr) {
    if (netPoints.length) break;
    const groupArr = Array.isArray(group) ? group : [group];
  ...
  return netPoints.map((p) => ({
    ...
    amount: valueMode === 'streams' ? 0 : p.y ?? 0,
    streams: valueMode === 'streams' ? Math.round(p.y ?? 0) : 0,
  }));
}
...
function hasData(arr) {
  return Array.isArray(arr)
    && arr.length > 0
    && arr.some((item) => (item.amount || 0) > 0 || (item.streams || 0) > 0);
}
...
async function loadArtistChartsDashboard(user) {
  ...
  const mappedArtistId = user.datalensArtistId || null;
  ...
  if (artistEmbedId && mappedArtistId) {
    ...
    byPeriodRaw = parseTimeSeriesChart(byPeriodR, 'streams');
    byTrackRaw  = parseBarChart(byTrackR);
    if (hasData(byTrackRaw) || hasData(byPeriodRaw)) linked = true;
  }

  // 2. Try admin charts — with artist_id param if mapped, otherwise by name
  if (!linked && adminEmbedId) {
    const adminParams = mappedArtistId ? params : {};
    const [byArtistR, byTrackAdminR, byPeriodAdminR] = await Promise.all([
      runChart(adminEmbedId, ADMIN_CHART_IDS.byArtist, adminParams),
      mappedArtistId ? runChart(adminEmbedId, ADMIN_CHART_IDS.byTrack, adminParams) : Promise.resolve(null),
      mappedArtistId ? runChart(adminEmbedId, ADMIN_CHART_IDS.byPeriod, adminParams) : Promise.resolve(null),
    ]);
    const adminArtists = parseBarChart(byArtistR);

    let matchedArtist = null;

    if (mappedArtistId && adminArtists.length > 0) {
      // artist_id param filters the chart to only this artist's rows
      matchedArtist = adminArtists[0];
    } else {
      const emailPrefix = normalizeName(String(user.email || '').split('@')[0]);
      const userName    = normalizeName(user.name);
      matchedArtist = adminArtists.find((a) => {
    ...
    if (matchedArtist) {
      ...
      if (byTrackAdminR) {
        byTrackRaw = parseBarChart(byTrackAdminR);
      }
      if (byPeriodAdminR) {
        byPeriodRaw = parseTimeSeriesChart(byPeriodAdminR, 'streams');
      }
    }
  }
  ...
  byPeriodRaw.forEach((p) => {
    const cur = monthMap.get(p.period) || { period: p.period, amount: 0, streams: 0 };
    cur.amount += p.amount;
    cur.streams += p.streams || 0;
    monthMap.set(p.period, cur);
  });
  const byMonth = Array.from(monthMap.values())
    .sort((a, b) => a.period.localeCompare(b.period))
    .map((p) => ({ period: p.period, amount: roundMoney(p.amount), streams: p.streams }));
  const totalStreams = byMonth.reduce((sum, item) => sum + (item.streams || 0), 0);
  const byTrack = byTrackRaw.map((t, i) => ({
    trackId:    `dl-${i}`,
    trackTitle: t.name,
    amount:     roundMoney(t.amount),
  ...
  // Fetch streams by platform from admin chart filtered by artist_id
  let bySource = [];
  if (adminEmbedId && mappedArtistId) {
    try {
      const bySourceR = await runChart(adminEmbedId, ADMIN_CHART_IDS.bySource, params);
      bySource = parseStackedBarBySource(bySourceR).map((item) => ({
        source: item.source,
        streams: item.streams,
        amount: 0,
      }));
