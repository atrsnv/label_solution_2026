...
function normalizeDatalensRows(payload) {
  ...
  return rows.map((row) => {
    const artistEmail = String(row.artistEmail || row.artist_email || '');
    const artistEmailName = artistEmail ? artistEmail.split('@')[0] : '';
    return {
      transaction_id: String(row.transaction_id || row.transactionId || row.id || ''),
      date: String(row.date || row.period || row.createdAt || ''),
      track_id: String(row.track_id || row.trackId || row.trackTitle || row.track_title || ''),
      track_title: String(row.track_title || row.trackTitle || row.title || ''),
      artist_id: String(row.artist_id || row.artistId || artistEmail || ''),
      artist_name: String(row.artist_name || row.artistName || artistEmailName || ''),
      role: String(row.role || 'Артист'),
