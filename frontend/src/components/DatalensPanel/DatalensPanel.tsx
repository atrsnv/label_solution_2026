...
const DatalensPanel = ({
  ...
  return (
    <section className={`datalens-panel ${className}`.trim()}>
      ...
      {!isLoading && !isError && primaryList.length === 0 && periodList.length === 0 && sourceList.length === 0 && (
        <p className="datalens-panel__message">
      ...
      {!isLoading && !isError && (primaryList.length > 0 || periodList.length > 0 || sourceList.length > 0) && (
        <div className="datalens-panel__grid">
          <article className="datalens-panel__chart">
            <h3>{isArtist ? 'Доход по трекам' : 'Доход по артистам'}</h3>
          ...
          <article className="datalens-panel__chart">
            <h3>{isArtist ? 'Прослушивания по платформам' : 'Доход по периодам'}</h3>

            {isArtist ? (
              sourceList.length > 0 ? (
                <div className="datalens-panel__bars">
                  {sourceList.slice(0, 6).map((item) => (
                    <div className="datalens-panel__bar" key={item.source}>
                      <div className="datalens-panel__bar-head">
                        <span>{item.source}</span>
                        <strong>{item.streams.toLocaleString('ru-RU')}</strong>
                      </div>
                      <div className="datalens-panel__bar-track">
                        <i style={{ width: `${Math.max((item.streams / maxSource) * 100, 6)}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <p className="datalens-panel__message">Нет данных по платформам</p>
              )
            ) : (
